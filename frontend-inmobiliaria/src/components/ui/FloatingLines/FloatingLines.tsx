...
// COLORES CORPORATIVOS PARA EL FONDO DEL SHADER
const vec3 BG_COLOR = vec3(0.0, 43.0, 91.0) / 255.0; // Un azul muy oscuro
const vec3 LINE_BASE_COLOR = vec3(255.0, 255.0, 255.0) / 255.0; // Líneas blancas/celestes
...
vec3 background_color(vec2 uv) {
  return BG_COLOR; // Fondo sólido corporativo
}

vec3 getLineColor(float t, vec3 baseColor) {
  if (lineGradientCount <= 0) {
    return LINE_BASE_COLOR;
  }
  vec3 gradientColor;
  ...
  return gradientColor * 0.4; // Ajuste de brillo de las líneas
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Mezclamos las líneas con el fondo base
  fragColor = vec4(col + b, 1.0);
}
